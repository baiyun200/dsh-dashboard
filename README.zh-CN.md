...
使用 **shadcn/ui** + **Recharts** 构建，中英文双语界面：统计卡片、图表、分类总览、可搜索/筛选/排序的插件表格与详情抽屉。首次进入自动跟随浏览器语言，也可随时在右上角切换。
...
node scripts/smoke-test.mjs   # puppeteer-core + 本机 Chrome 的端到端冒烟测试
```
...
src/
  ...
  lib/             # 类型 / 数据工具 / GitHub 实时刷新 / 主题 / 国际化
scripts/
  build-data.mjs   # 数据构建
  fetch-data.sh    # 数据抓取
